...
def parse_page_cycle_v(id_: str, group_id: int, driver, platform=None, media=None, ip=None, url=None):
    db_session.global_init("databases/accounts.db")
    if url is None:
        url_with_filters = \
            f"https://www.facebook.com/ads/library/?active_status=all&ad_type=all&country=ALL&" \
...
def create_parse_driver(profile_ids_list, cur_profile_idx=0):
    req_url_start = f"http://localhost:3001/v1.0/browser_profiles/{profile_ids_list[cur_profile_idx]}/start?automation=1"
